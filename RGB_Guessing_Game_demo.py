...
"""
TODO:
	- mode-ok elkeszitese (easy, medium, hard, impossible) (impossible DONE)
		- easy: csak 64-esével lehet tippelni (0, 63, 127, 191, 255)
		- medium: csak 32-esével
		- hard: csak 16-osával
		- impossible: az összes
	- scoreboard elkeszitese (.txt alapu maybe?) (DONE --> csv alapu lett)
	- pontszamok kerekitese (maybe?)
	- esztetikumot varazsolni a feluletre
	- minden tip utan mutassa, hogy hogy nez ki az a szin amire tippelt, ahhoz kepest, ami volt valojaban --> DONE
"""
...
class MainWindow(tk.Tk):
	def __init__(self):
		...
		if not os.path.exists('scoreboard.csv'):
			f = open('scoreboard.csv', 'x')
			f.write('nev,pont,datum\n')
			f.write(f'sample,0,{datetime.datetime.now().strftime("%Y-%m-%d-%H-%M-%S")}\n')
			f.close()
		...
		self.listbox_difficulty.configure(state=tk.DISABLED)
		self.wip_text = tk.Label(self, text='WIP', font=('italic', 14), fg='red')
		self.wip_text.place(x=260, y=150)

		# button scoreboard
		self.button_scoreboard = tk.Button(self, text='SCOREBOARD', font=('italic', 15), command=self.load_scoreboard)
		self.button_scoreboard.place(x=180, y=330)

		# button exit game
		self.button_exit_game = tk.Button(self, text='EXIT GAME', font=('italic', 15), command=self.destroy)
		self.button_exit_game.place(x=180, y=400)

		# button reset game
		self.button_reset_game = tk.Button(self, text='RESET START GAME', font=('italic', 10), command=self.reset_game).place(x=180, y=450)
...
class ScoreboardWindow(tk.Frame):
	def __init__(self):
		...
		self.parent = tk.Tk()
		self.parent.geometry('500x550+500+200')
		tk.Frame.__init__(self)
		self.main = self.parent
		self.main.geometry('500x500+500+200')
		self.main.title('SCOREBOARD')
		f = tk.Frame(self.main)
		f.pack(fill=tk.BOTH, expand=1)
		self.table = pt = Table(f)
		pt.show()
		pt.importCSV('scoreboard.csv')
		pt.sortTable(1, ascending=False)
		pt.redraw()

		self.button_exit = tk.Button(self.parent, text='BACK', font=('italic', 10), command=self.parent.destroy)
...
class GameWindowImpossible(tk.Tk):
	...
	def next_round(self):
		...
		r = self.entry_red.get()
		g = self.entry_green.get()
		b = self.entry_blue.get() 

		r = min(255, int(r)) if r else 0
		g = min(255, int(g)) if g else 0
		b = min(255, int(b)) if b else 0


		self.guesses[self.round] = [self.color_to_guess, [r, g, b]]
...
class EndScoreWindow(tk.Tk):
	def __init__(self, name, score):
		...
		# saving the score
		self.d = datetime.datetime.now().strftime('%Y-%m-%d-%H-%M-%S')
		with open('scoreboard.csv', 'a') as fp:
			writer = csv.writer(fp)
			writer.writerow([name, score, self.d])
